<x-app-layout>
    ...
    @section('breadcrumb')
        <x-breadcrumb :items="[
            ['title' => 'Reporting', 'url' => route('reporting.bills-by-user')],
            ['title' => 'Bills by User', 'url' => route('reporting.bills-by-user')],
        ]" />
    @endsection
    <!-- Content -->
            ...
            <div class="w-full" aria-label="body">
                ...
                <div class="mt-3 p-5">
                    <div class="divide-y divide-slate-300 flex flex-col gap-8" id="basic-tabs-1" role="tabpanel"
                        ...
                        <form class="flex flex-col gap-5" action="{{ route('reporting.bills-by-user') }}"
                            method="GET">
                            <div class="flex flex-row gap-5">
                                <span class="text-gray-400">Reporting name </span>
                                <span class="">Bills by User Report</span>
                            </div>
                        ...
                        <main class="p-5">
                            <table class="dt-report-bills-by-user display w-full"
                            data-title="{{ isset($reportFrom)?Str::of("Bills Reporting Summary for Users")->headline():'' }}"
                                ...
                                <tbody>
                                    @foreach ($users as $key => $rRow)
                                        <tr>
                                            ...
                                            @foreach ($feeTypes as $key => $row)
                                                @php
                                                    $colVal = $rRow->billReport($row, $reportFrom, $reportTo);
    ...
    <!-- End Content -->
    @section('script')
        <script src="{{ asset('js/reporting/bills-by-user.js') }} " defer></script>
    @endsection
