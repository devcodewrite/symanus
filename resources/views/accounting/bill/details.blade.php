            ...
            <div class="w-full" aria-label="body">
                ...
                <div class="mt-3 p-5">
                                ...
                                <div class="flex flex-row justify-between py-3">
                                    ...
                                    @if ($bill->payments())
                                        <span class="w-1/2 text-green-400 font-semibold">
                                            {{ __('GHS ') }} {{ number_format($bill->payments()->sum('amount'),2) }}
                                        </span>
                                    @else
                    ...
                    <div id="basic-tabs-2" class="{{ isset($payment)?'':'hidden' }}" role="tabpanel" aria-labelledby="basic-tabs-item-2">
                            ...
                            <div
                                ...
                                <div class="w-full">
                                    ...
                                    <x-input id="amount" class="mt-1 block w-full" type="number" name="amount"
                                        :value="old('amount', isset($payment) ? $payment->amount : '')" required autofocus placeholder="{{ __('Amount') }}" />
                                </div>
                              
                                <div class="w-full">
                                    <x-label for="paid_at" :value="__('Paid Date')" />
                                    <x-input id="paid_at" class="mt-1 block w-full" type="date" name="paid_at"
                                        :value="old('paid_at', isset($payment) ? $payment->paid_at : date('Y-m-d'))" required placeholder="{{ __('Paid Date') }}" />
                                </div>
                              
                                <div class="w-full">
                                    <x-label for="paid_by" :value="__('Paid by')" />
                                    <x-input id="paid_by" class="mt-1 block w-full" type="text" name="paid_by"
                                        :value="old('paid_by', isset($payment) ? $payment->paid_by : '')" required placeholder="{{ __('Paid by') }}" />
                                </div>
                        ...
                        @else
                        <div class="p-5">
                            <div class="p-5 w-full h-24 text-lg font-bold text-center flex items-center justify-center rounded-lg text-white bg-green-500">Payment completed!</div>
                        </div>
                        @endif
                                    ...
                                    @foreach ($bill->payments as $key => $row)
                                        <tr>
                                            ...
                                            <td class="border text-center flex items-end gap-3">
                                                <form
                                                    action="{{ route('payments.update', ['payment' => $row->id]) }}"
