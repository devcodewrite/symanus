<x-app-layout>
    ...
    @section('breadcrumb')
        <x-breadcrumb :items="[
            ...
            ['title' => 'Expense Details', 'url' => route('expenses.show', ['expense' => $expense->id])],
        ]" />
                ...
                <div class="mt-3 p-5">
                    <div class="divide-y divide-slate-300" id="basic-tabs-1" role="tabpanel"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10">
                            <div class="flex flex-row justify-between w-full md:col-span-2">
                                <div class="flex flex-row">
                                    ...
                                    <div class="mx-3">
                                        <p class="text-sky-600 font-semibold">{{ $expense->id }} </p>
                                        <p class="mt-2 font-semibold">{{ $expense->expenseType->title }} </p>
                                        <p class="uppercase text-gray-600"> {{ __('GHS ') }}  {{ $expense->amount }} </p>
                                    </div>
                                </div>
                                <div class="item-end">
                                    <span
                                        class="uppercase py-2 px-4 rounded text-white font-semibold shadow-md {{ ['open' => 'bg-green-600', 'close' => 'bg-red-600'][$expense->rstatus] }}">{{ $expense->rstatus }}</span>
                                </div>
                            </div>
                            ...
                            <div class="flex flex-col w-full divide-y divide-slate-300">
                                ...
                                <div class="flex flex-row justify-between py-3">
                                    ...
                                    <span class="w-1/2 text-blue-600 font-semibold">
                                       {{ __('GHS ') }} {{ $expense->amount }}
                                    </span>
                                </div>
                               
                                <div class="flex flex-row justify-between py-3">
                                    <span class="w-1/2 text-gray-600">Expense Class</span>
                                    <span class="w-1/2">
                                        {{ $expense->class->name }}
                                    </span>
                                </div>
                        ...
                        <div class="py-5">
                            ...
                            <x-a-button-p class="ml-3 py-3.5 item-end" :href="route('expenses.edit', ['expense' => $expense->id])">
                                {{ __('Modify') }}
                            </x-a-button-p>
