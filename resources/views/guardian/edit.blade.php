                ...
                <div class="w-full">
                    <div aria-label="formbody" class="mt-6 px-6 py-4 grid md:grid-cols-2 gap-6 overflow-hidden">
                        ...
                        <div class="w-full flex flex-col md:flex-row gap-6">
                            <div class="w-full field">
                                <x-label for="title" :value="__('Select the title:')" />
                                <x-select id="title" class="block w-full select2" name="title" required
                                    placeholder="Select the title">
                                    <option value=""></option>
                                   @foreach(['Mr.','Mrs','Master','Miss','Dr','Prof.'] as $key => $row)
                                   <option value="{{$row}}" {{ isset($guardian)? ($guardian->title ===  $row ? 'selected' : ''):'' }}>{{$row}}
                                    </option>
                                   @endforeach
                                </x-select>
                            </div>
                            ...
                            <div class="w-full field">
                                ...
                                <x-select id="sex" class="block w-full select2" name="sex" required
                        ...
                        <!-- Guardian Phone -->
                        <div class="w-full field">
                            <x-label for="phone" :value="__('Guardian Phone')" />
                            <x-input id="phone" class="mt-1 block w-full" type="text" name="phone"
                                :value="old('phone', isset($guardian) ? $guardian->phone : '')" placeholder="Enter the {{ __('Phone Number') }}" />
                        </div>

                           <!-- Guardian email -->
                           <div class="w-full field">
                            <x-label for="email" :value="__('Guardian email')" />
                            <x-input id="email" class="mt-1 block w-full" type="text" name="email"
                                :value="old('email', isset($guardian) ? $guardian->email : '')" placeholder="Enter the {{ __('Email') }}" />
                        </div>
